...
from fthd.model.models_supervised_ekf_51 import string_to_model, string_to_dataset
import torch
import numpy as np
import os
import yaml
    ...
    for i in range(model.epochs):
        ...
        for features, labels, time_feature, times, norm_inputs in train_data_loader:
            ...
            if model.is_rnn:
                h = h.data
            if model.is_rnn:
                output, o, _, h, acc_output = model(features, norm_inputs, labels,  h, time_feature, times)
            else:
                output, o, _, _, acc_output = model(features, norm_inputs, labels, None, time_feature, times)
        ...
        for val_f, val_la, val_timef, val_times, val_normin in val_data_loader:
            ...
            if model.is_rnn:
                val_h = val_h.data
                out, o, _, val_h, _ = model(val_f, val_normin, val_la, val_h,val_timef, val_times)
            else:
                out, o, _, _, _ = model(val_f, val_normin, val_la, None, val_timef, val_times)
        ...
        print("Epoch: {}/{}...".format(i+1, model.epochs),
            "Loss: {:.9f}...".format(mean_train_loss),
            "Val Loss: {:.9f}".format(mean_val_loss),
            "loss1: {:.9f}, loss2: {:.9f}, alpha1: {:.9f}, alpha2: {:.9f}".format(loss_items[0].item(),loss_items[1].item(),alpha_item[0].item(),alpha_item[1].item()))
    ...
    for i in range(additional_epoches):
        ...
        for features, labels, time_feature, times, norm_inputs in train_data_loader:
            ...
            if model.is_rnn:
                h = h.data
            if model.is_rnn:
                output, o, n, h, acc_output = model(features, norm_inputs, labels, h, time_feature, times)
            else:
                output, o, n,  _, acc_output = model(features, norm_inputs, labels, None, time_feature, times)
            # print("h shape :",h.shape)
            # grad_outputs = Variable(torch.ones(features.shape[0], 1), requires_grad=False).to(device)

            loss1 = model.weighted_mse_loss(o, labels, weights)
            labels = labels - n
        ...
        for val_f, val_la, val_timef, val_times, val_normin in val_data_loader:
            ...
            if model.is_rnn:
                val_h = val_h.data
                out, o, n, val_h, _ = model(val_f, val_normin, val_la, val_h,val_timef, val_times)
            else:
                out, o, n, _, _ = model(val_f, val_normin, val_la, None, val_timef, val_times)
